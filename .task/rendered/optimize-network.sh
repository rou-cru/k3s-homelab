...
if [ -n "$MAX_RX" ] && [ "$MAX_RX" -gt 256 ]; then
    if ethtool -G "$INTERFACE" rx "$MAX_RX" tx "$MAX_TX" 2>/dev/null; then
        logger -t network-optimization -p info "Ring buffers optimized: RX/TX=$MAX_RX/$MAX_TX"
    else
        logger -t network-optimization -p err "Ring buffers failed"
    fi
fi
