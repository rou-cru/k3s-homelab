...
Optional (if you want to use Ansible Vault): `task secrets:setup`
...
The goal is to extend the homelab with additional nodes connected via Tailscale
and orchestrated by K3s. Currently only the control plane (server) node is
automated; worker nodes remain to be added.
